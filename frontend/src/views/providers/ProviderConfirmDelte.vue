    ...
    if (error.response) {
      ...
      if (error.response.status === 400) {
        providerErrors.value = {
          message: 'El prestador no se puede eliminar porque tiene órdenes asociadas.'
        }
  ...
  <div class="row">
    ...
    <div class="col-md-4">
      ...
      <span v-if="providerErrors.message404">
        <p class="form-text text-danger">
          {{ providerErrors.message404 }}
        </p>
      </span>
      <div>
        <p>Está seguro que desea eliminar el prestador: {{ provider.first_name }}</p>
        <button class="btn btn-sm btn-danger" @click="delProvider(provider.id)">Eliminar</button>
        <RouterLink
          v-if="provider.id"
